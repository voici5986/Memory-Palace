...
- **The current validation snapshot is based on fresh reruns in this session**: backend tests are now `1071 passed, 22 skipped`; frontend is now `187 passed`; frontend build and typecheck both passed. This same session also reran a repo-local macOS `Profile B` browser smoke, reran repo-local live MCP e2e with a full `PASS` summary, and rechecked the Docker readiness/auth path: Dashboard `/` returned `200`, backend `/health` returned `200`, and protected setup/SSE requests stayed fail-closed. The narrower 2026-04-18 benchmark tables below were **not** recalculated in this final follow-up pass. Native Windows, native Linux host runtime paths, and Docker one-click `Profile C/D` still keep explicit target-environment recheck boundaries.
