...
- **The current validation snapshot is based on fresh reruns in this session**: backend tests `943 passed, 20 skipped`; frontend `159 passed`; frontend `npm run build` and `npm run typecheck` both passed. This same session also rechecked repo-local macOS `Profile B` (`backend + frontend + real browser setup/maintenance smoke`) and a local smoke pass covering the same retrieval / reranker / write-guard / gist paths as `Profile C/D`. Docker one-click `Profile C/D`, native Windows, and native Linux host runtime paths were **not** rerun in this round and still keep explicit target-environment recheck boundaries.
...
> In the same session, repo-local macOS `Profile B` (`backend + frontend + real browser setup/maintenance smoke`) and a local smoke pass covering the same retrieval / reranker / write-guard / gist paths as `Profile C/D` were also rerun. Docker one-click `Profile C/D` plus native Windows and native Linux host runtime paths were not rerun in this round.
...
- The Setup Assistant's local `.env` write path is stricter than that loopback read bypass: it only targets project-local `.env*` files, stays direct-loopback-only, and once the backend already has `MCP_API_KEY` configured, even loopback writes must carry a valid key
