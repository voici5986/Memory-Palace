...
- **The current validation snapshot is based on fresh reruns in this session**: backend `834 passed, 20 skipped`, frontend `125 passed` plus build success, live MCP-only e2e passed, the repo-local skill evaluator still exits successfully with environment-sensitive items kept as `PARTIAL` / `SKIP`, repo-local Profile B (`backend + SSE + Vite`) was rechecked, Docker one-click Profile B was rechecked end to end, and the real A/B/C/D benchmark was rerun against the current local model services. PowerShell profile generation was also rechecked through `smoke_apply_profile_ps1_in_docker.sh`. Native Windows host runtime and native Linux host runtime still keep explicit target-environment recheck boundaries.
- **Setup Assistant and Dashboard write affordances are less misleading now**: authenticated non-loopback requests can still inspect setup status, but local `.env` writes remain direct-loopback-only, so the save button now stays disabled with an explicit reason instead of looking writable and then failing. Review / Maintenance also fail closed with inline notices when `confirm` / `prompt` / `alert` are unavailable in the current runtime.
