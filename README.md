...
- **The current validation snapshot is based on fresh reruns in this session**: backend tests are now `1093 passed, 22 skipped`; frontend is now `193 passed`; frontend build and typecheck both passed. This same session also reran a repo-local macOS `Profile B` browser smoke, reran repo-local live MCP e2e with a full `PASS` summary, and rechecked the Docker readiness/auth path: Dashboard `/` returned `200`, backend `/health` returned `200`, and protected setup/SSE requests stayed fail-closed. Earlier in the same session, Docker one-click `Profile C/D` was also rechecked: the generated Docker env rewrites loopback LLM/router bases to `host.docker.internal`, keeps private embedding/reranker hosts on the explicit allowlist, and no longer emits the earlier `invalid embedding/reranker API base` warnings. The narrower 2026-04-18 benchmark tables below were **not** recalculated in this final follow-up pass. Native Windows and native Linux host runtime paths still keep explicit target-environment recheck boundaries.
...
>
> Follow-up note (2026-04-20): after the later review closeout fixes, the repository reran the full backend suite (`1093 passed, 22 skipped`), and the same session still kept the frontend rerun truth at `193 passed` with frontend build/typecheck passing. That same closeout note also kept the Docker readiness/auth recheck (`/` `200`, `/health` `200`, protected setup/SSE requests still fail-closed), the repo-local `Profile B` browser smoke, and repo-local live MCP e2e (`docs/skills/MCP_LIVE_E2E_REPORT.md`, full `PASS`). The narrower benchmark table above itself was not recalculated in that final follow-up pass.
