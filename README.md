...
- **The current validation snapshot is based on fresh reruns in this session**: backend tests `957 passed, 20 skipped`; frontend `164 passed`; frontend `npm run build` and `npm run typecheck` both passed. This same session also rechecked repo-local macOS `Profile B` (`backend + frontend + real browser setup/maintenance smoke`) and a local smoke pass covering the same retrieval / reranker / write-guard / gist paths as `Profile C/D`. Docker one-click `Profile C/D`, native Windows, and native Linux host runtime paths were **not** rerun in this round and still keep explicit target-environment recheck boundaries.
...
- Provider API bases written through the assistant are normalized and validated before save: common suffixes such as `/embeddings`, `/rerank`, and `/chat/completions` are trimmed, while malformed or link-local targets are rejected
