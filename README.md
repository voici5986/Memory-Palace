...
- **The current validation snapshot is based on fresh reruns in this session**: backend `870 passed, 20 skipped`; frontend `138 passed` plus build success; live MCP e2e passed again; the repo-local skill evaluator still exits cleanly with host-dependent items kept as `PARTIAL` / `SKIP`; repo-local macOS Profile B (`backend + SSE + Vite`) was rechecked through a real browser path; Docker one-click Profiles A/B/C/D were rechecked with healthy `/health`, proxied `/sse`, and protected API routes; and the real A/B/C/D benchmark matrix was rerun against the current local model services. Native Windows host runtime and native Linux host runtime still keep explicit target-environment recheck boundaries.
