...
- **The current `v3.7.1` validation is tighter where it matters**: backend `809 passed, 8 skipped`, frontend `119 passed` plus build success, live MCP-only e2e passed, repo-local skill sync check passed again, the repo-local skill evaluator now exits successfully with environment-sensitive items kept as `PARTIAL` / `MANUAL`, native Windows local backend/SSE/Vite/browser smoke was rechecked, and Windows-host Docker Profile B was rechecked through both manual `docker compose` and `docker_one_click.sh`. Native macOS host runs still keep explicit target-environment caveats in this release note.
