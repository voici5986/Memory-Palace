...
- **The current validation snapshot is based on fresh reruns in this session**: backend tests are now `1111 passed, 22 skipped`; frontend is now `194 passed`; frontend build and typecheck both passed. This same session also reran a repo-local macOS `Profile B` browser smoke, reran repo-local live MCP e2e with a full `PASS` summary, rechecked the Docker readiness/auth path (`/` = `200`, `/health` = `200`, protected setup/SSE requests still fail-closed), and completed a smaller real A/B/C/D rerun on `BEIR NFCorpus` (`sample_size=5`, `Profile D` Phase 6 Gate still `PASS`). Earlier in the same session, Docker one-click `Profile C/D` was also rechecked: the generated Docker env rewrites loopback LLM/router bases to `host.docker.internal`, keeps private embedding/reranker hosts on the explicit allowlist, and no longer emits the earlier `invalid embedding/reranker API base` warnings. The narrower 2026-04-18 benchmark tables below were **not** recalculated in this final follow-up pass. Native Windows and native Linux host runtime paths still keep explicit target-environment recheck boundaries.
- **Public MCP contracts are stricter now**: the MCP boundary now rejects control/invisible/surrogate URI characters, blocks overlong `search_memory` / `create_memory` / `update_memory` payloads before DB work starts, and `add_alias` now rolls back the alias path if snapshot capture fails after the DB write.
...
>
> Follow-up note (2026-04-21): after the regression-fix reruns, the repository reran the full backend suite (`1111 passed, 22 skipped`), and the same session kept the frontend rerun truth at `194 passed` with frontend build/typecheck passing. That same follow-up also kept the Docker readiness/auth recheck (`/` `200`, `/health` `200`, protected setup/SSE requests still fail-closed), the repo-local `Profile B` browser smoke, repo-local live MCP e2e (`docs/skills/MCP_LIVE_E2E_REPORT.md`, full `PASS`), and a smaller real A/B/C/D rerun on `BEIR NFCorpus` (`sample_size=5`, `Profile D` gate still `PASS`). The narrower benchmark table above itself was not recalculated in that final follow-up pass.
