...
- **The current validation snapshot is based on fresh reruns in this session**: backend tests `1007 passed, 22 skipped`; frontend `172 passed`; frontend `npm run build`, `npm run typecheck`, and `npm run test:bundle-budget` all passed; the main compose / GHCR / override config checks also passed. This same session also reran isolated live probes for the local Profile B HTTP/proxy path and a real local C-path retrieval check (`1024`-d embedding + reranker). Repo-local live MCP e2e, native Windows, native Linux host runtime paths, and Docker one-click `Profile C/D` were **not** rerun in this round and still keep explicit target-environment recheck boundaries.
...
- **Reflection rollback is less ambient and more auditable now**: reflection rollback now requires an explicit `session_id`, verifies that `session_id` (and `actor_id` when supplied) against the learn job before deleting anything, and records one `reflection_workflow` rollback event instead of double-counting. If the execute path has to roll back through a review snapshot, the backend now also does best-effort namespace cleanup for the auto-created reflection path.
...
- **Dashboard loading and local SSE path resolution are cleaner now**: the larger Dashboard routes now load lazily, the frontend bundle budget is back under the warning threshold, and the lightweight `EventSource` helper now resolves `/sse` against a prefixed `VITE_API_BASE_URL` instead of always assuming the site root.
