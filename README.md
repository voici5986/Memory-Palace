...
- **Repo-local wrapper rejects normalized Docker sqlite paths more consistently now**: the built-in Python and shell wrappers now normalize common slash and case variants before deciding whether a local `DATABASE_URL` still points at Docker-internal `/app/...` or `/data/...` paths, so values like `sqlite+aiosqlite://///app/data/...` are no longer accepted by accident.
- **The current validation snapshot is based on fresh reruns in this session**: backend non-benchmark tests `868 passed, 15 skipped`; frontend `154 passed`; frontend `npm run typecheck` and build both passed. This same session also rechecked repo-local macOS `Profile B` (`backend + SSE + Vite + browser + i18n persistence`), Docker one-click Profiles A/B/C/D (`/health`, frontend root, `/sse`, and proxied `/api/browse/node`), and a narrower real A/B/C/D benchmark rerun on `squad_v2_dev` with `sample_size=2`. `skills+MCP` and `single-MCP` passed in this round; `skills-only` is still explicitly **PARTIAL**. Native Windows and native Linux host runtime paths were **not** rerun in this round and still keep explicit target-environment recheck boundaries.
...
- **Reflection prepare is less likely to duplicate work now**: concurrent `prepare` requests with the same session, source, reason, and content now reuse the same prepared review instead of minting multiple review IDs for the same batch.
- **Setup Assistant is less misleading now**: authenticated non-loopback requests can still inspect setup status, but local `.env` writes remain direct-loopback-only, so the save button now stays disabled with an explicit reason instead of looking writable and then failing. `/setup/status` support probing is now side-effect-free and no longer creates missing parent directories just to answer capability checks. Delayed setup-status hydration now also preserves untouched retrieval fields, so typing the Dashboard key first no longer resets an existing router/reranker setup back to `hash` / `false`. `Profile B/C/D` preset switches and related retrieval toggles now clear hidden stale fields before save, real local router bases such as `http://127.0.0.1:8001/v1` stay valid, example router model IDs are still treated as placeholders, switching from local hash to a remote embedding backend now writes the correct `RETRIEVAL_EMBEDDING_DIM` instead of leaving the old `64` behind, and direct `api` / `openai` embedding paths now also require a real positive integer dimension before local `.env` save. If you choose **Save dashboard key only**, that browser-side key now takes effect immediately for protected Dashboard requests in the current page, and the success banner stays visible until you close the dialog yourself. The current dialog also focuses the Dashboard API key field automatically, supports `Escape` to close, and keeps `Tab` / `Shift+Tab` inside the dialog until you close it.
...
When neither runtime Dashboard auth nor stored browser Dashboard auth is available, the frontend auto-opens a first-run setup assistant. It can save the Dashboard `MCP_API_KEY` in the current browser session and, when the app is running directly against a local checkout, write the common local runtime fields into `.env` without hand-editing the file. The assistant now also exposes `Profile A` explicitly instead of leaving it implicit in the empty form state, and that baseline still means `keyword + none`. If you use the local `.env` save path and also enter a Dashboard key, the assistant still needs browser session storage for that key; if the browser blocks that storage path, the page now shows a save failure instead of pretending the whole setup succeeded. On authenticated non-loopback requests, the assistant can still show the current setup status, but the local `.env` save path stays disabled with an explicit loopback-only reason. On the trusted Docker / GHCR proxy path, the assistant also no longer auto-opens just because the browser itself does not hold the key; if protected requests already work through the proxy, the page stays on the normal Dashboard flow. Backend-side changes still require a restart.
...
> In the same session, repo-local macOS `Profile B` (`backend + SSE + Vite + browser + i18n persistence`) and Docker one-click Profiles A/B/C/D (`/health`, frontend root, `/sse`, and proxied `/api/browse/node`) were also rerun successfully. Native Windows and native Linux host runtime paths were not rerun in this round.
