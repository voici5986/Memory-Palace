...
> Verification note for this session: backend reruns were `966 passed / 20 skipped`; frontend reruns were `165 passed`; frontend `typecheck` and build both passed, and the repo-local live MCP e2e also passed. This same session also rechecked repo-local macOS `Profile B` (`backend + frontend + real browser setup/maintenance smoke`) and a local smoke pass covering the same retrieval / reranker / write-guard / gist paths as `Profile C/D`. Docker one-click `Profile C/D`, native Windows, and native Linux host runtime paths were not rerun in this round and still keep explicit target-environment recheck boundaries.
