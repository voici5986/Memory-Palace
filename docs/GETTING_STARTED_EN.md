...
>
> If you deploy the frontend under a non-root prefix and also set `VITE_API_BASE_URL`, the shared frontend SSE helper now resolves `/sse` against that prefixed API base as well, instead of always assuming the site root. Without browser-side auth it still behaves like a native `EventSource` path; with browser-side auth it switches to fetch-based SSE so the client can send auth headers and reconnect after a non-terminal drop. If the browser's Dashboard auth changes at runtime, the next non-terminal reconnect also re-reads the current key / mode. That fetch-based path now also pauses/resumes around `visibilitychange` / `pagehide` / `pageshow` / `beforeunload`, uses exponential-backoff retries, and aborts idle half-open streams with an idle watchdog.
>
