...
>
> If you deploy the frontend under a non-root prefix and also set `VITE_API_BASE_URL`, the lightweight frontend `EventSource` helper now resolves `/sse` against that prefixed API base as well, instead of always assuming the site root.
>
