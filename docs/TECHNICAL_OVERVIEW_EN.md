...
- Reflection rollback now also requires an explicit `session_id`; the backend checks that `session_id` (and `actor_id` when supplied) against the learn job before it deletes anything instead of silently falling back to an ambient session.
...
├── lib/sse.js                                 # Lightweight EventSource helper reused for local `/sse` debugging
├── locales/
│   ├── en.js                                  # English copy
...
- The frontend now also has a lightweight `EventSource` helper (`frontend/src/lib/sse.js`); the repo-local Vite `/sse` proxy is intentionally still kept as the same-origin EventSource / MCP debugging path. If the frontend uses a prefixed `VITE_API_BASE_URL`, that helper now resolves `/sse` against the same prefix instead of assuming the site root.
...
- Compose file: `docker-compose.yml` (before probing `127.0.0.1:8080`, the frontend healthcheck now explicitly unsets the inherited `http_proxy/https_proxy/all_proxy/no_proxy` family so a container-side localhost probe does not get misrouted through a proxy chain)
...
- The local `/setup/config` write path is also fail-closed: it only targets project-local `.env*` files, still requires a direct loopback request, the first local save also requires a non-empty Dashboard key, and if the backend already runs with `MCP_API_KEY`, that loopback write also requires the same valid key.
