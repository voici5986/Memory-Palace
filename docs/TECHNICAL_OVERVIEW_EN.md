...
- If the caller explicitly sends a blank or whitespace-only `session_id`, the reflection workflow now fails closed with `session_id_invalid`; it only falls back to the ambient session when `session_id` is truly omitted.
...
- The Setup Assistant now also shows `Profile A` explicitly; it still means the default `keyword + none` baseline rather than a new higher-tier profile.
...
- The frontend now also has a lightweight `EventSource` helper (`frontend/src/lib/sse.js`); the repo-local Vite `/sse` proxy is intentionally still kept as the same-origin EventSource / MCP debugging path.
