...
The current validate path now treats frontend typecheck as a first-class check alongside frontend tests and the frontend build; in this session, the fresh reruns were backend `1136 passed, 22 skipped`, frontend `198 passed`, passing frontend build/typecheck, and repo-local live MCP e2e (`PASS`). Earlier in the same session, a repo-local macOS `Profile B` real-browser smoke, a Docker readiness/auth recheck (`/` `200`, `/health` `200`, with protected setup/SSE requests still fail-closed), and a smaller real A/B/C/D rerun on `BEIR NFCorpus` with `Profile D` Phase 6 Gate still `PASS` were also completed. The narrower 2026-04-18 benchmark table was not recalculated in this final doc-sync pass, and Docker one-click `Profile C/D` plus native Windows / native Linux host runtime paths still keep explicit target-environment recheck boundaries here.
