...
- **`setup.py`**: First-run setup and local `.env` write entrypoint. `/setup/status` and `/setup/config` now distinguish between a real process-level explicit override and a value that was only loaded from `.env` during startup. In practice, setup status/save no longer misreads startup `.env` values as process overrides, and saving the local `.env` can refresh the current process view of setup-managed keys.
...
| `POST` | `/maintenance/learn/reflection` | Trigger the reflection workflow (`prepare/execute`; execute now enters the write lane, registers a learn job, and reuses the same rollback API) |
...
The current validate path now treats frontend `npm run typecheck` as a first-class check alongside `npm test` and the frontend build; in this session, the real reruns were backend non-benchmark `857 passed, 15 skipped`, frontend `149 passed`, and passing frontend typecheck/build, while live MCP e2e, real-browser paths, and Docker profiles were not rerun in this round.
