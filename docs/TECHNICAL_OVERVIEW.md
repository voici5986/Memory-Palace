...
- 前端现在还额外抽出了一层轻量 SSE helper（`frontend/src/lib/sse.js`）；repo-local Vite 的 `/sse` 代理继续保留。本地没有浏览器侧 Dashboard 鉴权时，它仍走同源原生 `EventSource`；如果浏览器已经有 Dashboard 鉴权，Observability 会切到 fetch-based SSE，这样同一把 header/bearer key 也能带到 `/sse`，临时断流后还会自动重连；遇到明确的 `4xx` 鉴权失败则不会盲目重试。带前缀的 `VITE_API_BASE_URL` 也会按同一个前缀去解析 `/sse`
