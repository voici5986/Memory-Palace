...
If you are using **Save local `.env` settings**, remember two current boundaries: the first local save requires a non-empty Dashboard key, and provider API base fields should point at the service base, not concrete suffixes such as `/embeddings`, `/rerank`, or `/chat/completions`. Those common suffixes are normalized automatically, but malformed or link-local targets are rejected.
