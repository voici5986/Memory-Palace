...
> 2026-04-20 round reran the full backend suite (`1071 passed, 22 skipped`),
> frontend tests (`187 passed`), frontend build, frontend typecheck,
...
> need a fixed location, pass `artifact_dir` explicitly. That directory is
> ignored by default so parallel benchmark runs do not dirty the worktree.
...
`backend/tests/benchmark/artifacts/<run-token>/profile_ab_metrics.json`
...
`backend/tests/benchmark/artifacts/<run-token>/profile_abcd_real_metrics.json`
...
`backend/tests/benchmark/artifacts/<run-token>/write_guard_quality_metrics.json`
...
**Source**: the latest run-scoped artifact under
`backend/tests/benchmark/artifacts/<run-token>/intent_accuracy_metrics.json`
(typically generated by benchmark helpers during the maintenance phase)
...
**Source**: the latest run-scoped artifact under
`backend/tests/benchmark/artifacts/<run-token>/compact_context_gist_quality_metrics.json`
(typically generated by benchmark helpers during the maintenance phase)
...
**Source**: the latest run-scoped artifact under
`backend/tests/benchmark/artifacts/<run-token>/prompt_safety_contract_metrics.json`
(typically generated by benchmark helpers during the maintenance phase)
...
**Source**: the latest run-scoped artifact under
`backend/tests/benchmark/artifacts/<run-token>/reflection_lane_metrics.json`
(typically generated by benchmark helpers during the maintenance phase)
...
- Full non-benchmark backend suite: `1071 passed / 22 skipped`
- Full frontend suite: `187 passed`
...
- `skills+MCP` / `single-MCP`: skill smoke was rerun; `claude` / `codex` / `gemini` passed, `cursor` / `agent` / `antigravity` remain `PARTIAL`, and `gemini_live` stays `SKIP`. `OpenCode` hit one timeout in the full multi-CLI sweep but passed on the immediate standalone rerun, so this is better understood as host-side fluctuation rather than a stable all-host `PASS` claim
