...
- **当前 `v3.7.1` 的验证更偏向真正关键的链路**：后端 `809 passed, 8 skipped`，前端 `119 passed` 且 build 成功，live MCP-only e2e 通过，repo-local skill sync check 重新回到通过状态，repo-local skill evaluator 现在也能在把环境敏感项保留为 `PARTIAL` / `MANUAL` 的前提下成功退出；native Windows 本机的 backend/SSE/Vite/浏览器 smoke 已复验，Windows 宿主上的 Docker Profile B 也已经按手动 `docker compose` 和 `docker_one_click.sh` 两条路径重新复验。native macOS 主机实跑在这次发布说明里仍然保留目标环境复验边界。
